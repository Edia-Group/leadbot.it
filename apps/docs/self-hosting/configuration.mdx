...
The webhook logic block uses long-polling on the **viewer** (`GET /api/v1/webhook-wait/{room}`) while the chat waits for an incoming webhook. The `POST .../executeWebhook` endpoints on the same viewer process resolve that wait. For a single-instance deployment this works without extra services; if you scale the viewer to multiple replicas, webhook delivery must hit the same instance that is holding the open wait (for example with sticky sessions).
